...
def plot_results(dataset: str, df: pd.DataFrame):
    ...
    #---------------------------------------------------------#
    ax.legend(loc='upper right', ncol=2, fontsize=fontsize, handlelength=2.0)

    plt.tight_layout()

    # Save the figure with both plots
    out_path = os.path.join(PROJDIR, 'figs', f'abl_num_blocks_{dataset}.pdf')
...
def add_job_to_queue(
    ...
    exp_name = os.path.join(f'abl_num_blocks_{dataset}', exp_name)
